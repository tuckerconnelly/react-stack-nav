...
payload:{stateObj:stateObj||{},title:title,url:url}};};
...
if(_url===state.history[state.history.length-1].url)return state;

var _stateObjWithIndex=_extends({},_stateObj,{index:state.index});

if(__WEB__)history.replaceState(_stateObj,_title,_url);
return {
index:state.index,
history:state.history.
slice(0,state.index).
